...
echo "$($PSQL "SELECT DISTINCT(name) FROM games AS winners FULL JOIN teams on winners.winner_id = teams.team_id FULL JOIN games AS opponents ON teams.team_id = opponents.opponent_id WHERE (winners.year = 2014 AND winners.round = 'Eighth-Final') OR (opponents.year = 2014 AND opponents.round = 'Eighth-Final') ORDER BY name")"
